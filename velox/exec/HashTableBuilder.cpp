...
struct JoinTableConfig {
  bool ignoreNullKeys;
  bool allowDuplicates;
  bool hasProbedFlag;
};

JoinTableConfig computeJoinTableConfig(
    core::JoinType joinType,
    bool nullAware,
    bool hasFilter) {
  if (core::isRightJoin(joinType) || core::isFullJoin(joinType) ||
      core::isRightSemiProjectJoin(joinType)) {
    return JoinTableConfig{false, true, true};
  }

  const bool dropDuplicates = !hasFilter &&
      (core::isLeftSemiFilterJoin(joinType) ||
       core::isLeftSemiProjectJoin(joinType) || core::isAntiJoin(joinType));
  const bool needProbedFlag = core::isRightSemiFilterJoin(joinType);
  const bool leftNullAwareWithFilter = hasFilter && nullAware &&
      (core::isAntiJoin(joinType) || core::isLeftSemiProjectJoin(joinType) ||
       core::isLeftSemiFilterJoin(joinType));

  return JoinTableConfig{
      !leftNullAwareWithFilter,
      !dropDuplicates,
      needProbedFlag};
}
...
HashTableBuilder::HashTableBuilder(
  ...
  const auto tableConfig =
      computeJoinTableConfig(joinType_, nullAware, hasFilter);
  ignoreNullKeys_ = tableConfig.ignoreNullKeys;
  allowDuplicates_ = tableConfig.allowDuplicates;
  hasProbedFlag_ = tableConfig.hasProbedFlag;
