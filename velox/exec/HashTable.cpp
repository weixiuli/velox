...
std::unique_ptr<BaseHashTable> BaseHashTable::deserialize(
  ...
  VELOX_CHECK(
      !serialized.metadataJson.empty(),
      "Serialized hash table metadata must not be empty");

  folly::dynamic metadata;
  try {
    metadata = folly::parseJson(serialized.metadataJson);
  } catch (const std::exception& e) {
    VELOX_FAIL(
        "Failed to parse hash table metadata JSON: {}", std::string(e.what()));
  }
  VELOX_CHECK(metadata.isObject(), "Hash table metadata must be a JSON object");
  VELOX_CHECK(
...
HashTableSerializedData HashTable<ignoreNullKeys>::serialize() const {
  ...
  opts.sort_keys = true;
  serialized.metadataJson = folly::json::serialize(metadata, opts);
  return serialized;
}
